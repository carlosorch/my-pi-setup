...
export const ChainItem = Type.Object(
	{
		...
		parallel: Type.Optional(
			Type.Unsafe({
				anyOf: [Type.Array(ParallelTaskSchema, { minItems: 1, description: "Tasks to run in parallel" }), DynamicParallelTemplateSchema],
...
const ControlOverrides = Type.Object({
	...
	failedToolAttemptsBeforeAttention: Type.Optional(
		Type.Integer({ minimum: 1, description: "Consecutive mutating-tool failures before escalating to needs_attention (default: 3)" }),
	),
...
const SubagentParamsSchema = Type.Object({
	...
	message: Type.Optional(
		Type.String({ description: "Follow-up message for action='resume' or non-terminal guidance for action='steer'. Use index to choose a child from multi-child runs." }),
	),
	scope: Type.Optional(
		Type.String({
			enum: ["session", "user", "project"],
			description: "Scope for action='watchdog.configure'. Defaults to session to avoid persistent settings writes unless user/project is explicit.",
		}),
	),
	target: Type.Optional(
		Type.String({
			enum: ["main", "children", "child"],
			description: "Target for action='watchdog.configure'. Defaults to main. Use target='child' with agent for a per-agent child watchdog override.",
		}),
	),
	thinking: Type.Optional(
		Type.Unsafe({
			...
			description: "Thinking override for execution or action='watchdog.configure' (off/minimal/low/medium/high/xhigh/max, inherit, or false for off).",
		}),
	),
	...
	chain: Type.Optional(
		Type.Array(ChainItem, { description: "CHAIN mode: sequential steps; each result becomes {previous}. append-step takes one tail step and may use {chain_dir}/{outputs.name}." }),
	),
